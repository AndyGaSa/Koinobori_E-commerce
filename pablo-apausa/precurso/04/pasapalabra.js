...
let scoreboard = [ 
	{username: 'carlen', score: 19},
	{username: 'essie', score: 13},
	{username: 'pierson', score: 11},
	{username: 'bryna', score: 23},
	{username: 'hardy', score: 8},
	{username: 'itzaak', score: 16},
];

let questions = []; 

function pasapalabra(username) {
	const empty = questions => questions.status === '';
	let count = 0; 

	//Generates the questions. 
	for (let i = 0; i < 27; i++) {
		let number = Math.round(Math.random() * 2);
		if (number === 0) questions.push(arr0[i]);
		if (number === 1) questions.push(arr1[i]);
		if (number === 2) questions.push(arr2[i]);
	}
	//Starts the game. 
	while (questions.filter(empty).length !== 0) {
		unanswered = questions.filter(empty); 
		let answer = prompt(unanswered[count].question); 
		if (answer === null || answer === "end") return;
		answer = answer.toLowerCase();
		if (answer !== "pasapalabra") {
			let index = questions.findIndex(questions => questions.letter === unanswered[count].letter);
			if (answer === unanswered[count].answer) { 
				questions[index].status = true;
				alert(`Correcto!`);
			} else {
				questions[index].status = false;
				alert(`Respuesta: ${unanswered[count].answer}.`);
			}
		} else count++; 
		if (count === questions.filter(empty).length) count = 0; 
	}
	//Logs the scoreboard.  
	let score = questions.filter(questions => questions.status === true).length;  
	scoreboard.push({username: username, score: score});
	scoreboard.sort((a, b) => a.score - b.score);
	scoreboard.reverse();
	console.log('Tabla de puntaciones:');
	for (let k = 0; k < scoreboard.length; k++) {
		console.log(`${scoreboard[k].username}: ${scoreboard[k].score}pts.`); 
	}
	alert(`Respuestas correctas: ${score}/27pts`);
}
...
function login() { 
	...
	if (!/[\w]/.test(username)) return login();
	console.clear(); 
