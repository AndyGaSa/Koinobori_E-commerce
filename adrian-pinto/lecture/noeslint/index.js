...
const login = () => {
  ...
  user.admin = adquisicionDatos('Introduce si eres Admin o User') === 'admin';
};
...
const adquisicionDatos = (mensaje, tipo) => {
  ...
  const evalChain = selectipo(tipo);
...
const verUltimosVuelos = () => {
  const vuelosActivos = flights.filter((flight) => flight.activo === true);
  const ultimosVuelos = vuelosActivos.slice(vuelosActivos.length - 5, vuelosActivos.length);
...
const crearVuelo = () => {
  let vuelosActivos = 0;
  const userInput = ['Porfavor introduce el nombre del origen.', 'Porfavor introduce el nombre del destino.', 'Porfavor introduce el coste del vuelo. ( Solo digitos )', 'El vueo tiene escalas? ( True | False )'];
...
/**
 * Busca el id unico del vuelo y si coincide con un vuelo activo afectua la compra,
 *  muestra un mensaje.
 * @param { Number } id
 * @returns { String }
 */
const comprarVuelo = (id) => (flights.find((flight) => flight.id === id && flight.activo === true) ? 'Gracias por su compra, vuelva pronto.' : 'El vuelo indicado no existe');
 ...
 */
const mostrarMenu = () => {
  const opciones = user.admin ? `06 - Crear vuelo
