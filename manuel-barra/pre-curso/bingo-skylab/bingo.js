...
const ObtenerNombre = () => {
    ...
    do {
        nombreInput = prompt(`Cuentanos tu nombre para registrar tu puntuación. `)
    } while (nombreInput === null)
...
const MostrarCarton = () => {
    let contMostrarCarton = 0
    while (contMostrarCarton < 15) {
...
const ConfirmarCarton = () => {
    ...
    while (respCarton === false) {
        respCarton = confirm(`Por favor ${nombreInput} Presiona Aceptar para jugar con este Carton o Cancelar para obtener otro.`)
        if (respCarton === true) {
            console.log(`Perfecto ${nombreInput} vamos a jugar...`)
...
const partidaBingo = () => {
    ...
    alert(`Vamos Jugar al Bingo ${nombreInput}`)
    do {
        ...
        if (bingoCarton[0] === "X" && bingoCarton[1] === "X" && bingoCarton[2] === "X") {
            ...
            console.log(`Estos son tus puntos. ${puntosInput}`)
        }

    } while (bingoCarton.some(Number))
    console.log(`Bingo ${nombreInput} has finalizado tu carton.`)
    alert(`Bingo ${nombreInput} has finalizado tu carton.`)
}
const guardarJugador = () => {
    ...
    var jug = new Jugador(nombreInput, puntosInput, contador.length)
    jugadores.push(jug)
...
const finalizarJuego = () => {
    ...
    if (fin === true) {
        ...
        let finFin = confirm(`Seguro deseas salir del juego ${nombreInput}.`)
        if (finFin === true) {
            alert(`Hemos terminado el juego ${nombreInput}, adios.`)
        } else {
            finalizarJuego()
        }
    }
